...
export class DIDDocument extends DIDEntity<DIDDocument> {
  ...
  public toJSON(): JSONObject {
    ...
    if (this.assertionMethods && this.assertionMethods.length > 0) {
      didDoc.assertionMethod = Array.from(this.assertionMethods, (m) => m.toJson());
    }

    if (this.authenticationMethods && this.authenticationMethods.length > 0) {
      didDoc.authentication = Array.from(this.authenticationMethods, (m) => m.toJson());
    }

    if (this.capabilityDelegationMethods && this.capabilityDelegationMethods.length > 0) {
      didDoc.capabilityDelegation = Array.from(this.capabilityDelegationMethods, (m) => m.toJson());
    }

    if (this.capabilityInvocationMethods && this.capabilityInvocationMethods.length > 0) {
      didDoc.capabilityInvocation = Array.from(this.capabilityInvocationMethods, (m) => m.toJson());
    }

    if (this.keyAgreementMethods && this.keyAgreementMethods.length > 0) {
      didDoc.keyAgreement = Array.from(this.keyAgreementMethods, (m) => m.toJson());
    }

    if (this.verificationMethods && this.verificationMethods.length > 0) {
      didDoc.verificationMethod = Array.from(this.verificationMethods, (m) => m.toJson());
    }
